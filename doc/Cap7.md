...
>En esta versión todavía no se encuentra soportada la gestión dinámica de *assets* lo que quiere decir que después de la carga de la página no se añadirá ningún nuevo *asset* aunque se inserte en la misma algún nuevo componente con dependencias a *assets* que no se encuentren cargados en la página.
...
No obstante, dado que por lo general solo es necesario redefinir los *assets* para modificar su URI, existe una manera muy simplificada de hacerlo. La solución consiste en crear un método en la página de nombre `rewriteUri()` y hacerlo devolver un *array* asociativo donde su llaves se corresponderán con el identificador del *asset* a modificar y su valor será la nueva URI.
